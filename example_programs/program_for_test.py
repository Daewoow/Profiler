...
def parse_datetime(parsed_dict):
    parsed_dict['DATETIME'] = datetime.strptime(parsed_dict['TIMESTAMP'], '%d/%b/%Y:%H:%M:%S %z')
    # print(parsed_dict)
    return parsed_dict
...
class MostPopularPage(Statistics):
    ...
    def result(self):
        result = most_common_lex_minimal(self.pages)
        return result if result is not None else "No data available"
...
class MostActiveClientByDay(Statistics):
    ...
    def result(self):
        print(self.stat_by_day.items())
        a = OrderedDict(sorted(
...
def norm_method():
    with open("example_programs/example_3.log", "r") as f:
...
if __name__ == '__main__':
    st_time = time.time()
    profiler.run('program_for_test.norm_method', sort=0)
